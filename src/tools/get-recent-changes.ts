		...
		return ctx.format.ok({
			changes: changes.map((c) => {
				...
				return {
					...
					userid: c.userid,
					...(c.anon ? { anon: true } : {}),
					...(c.userhidden ? { userhidden: true } : {}),
					...(c.commenthidden ? { commenthidden: true } : {}),
					revisionId: c.revid,
					oldRevisionId: c.old_revid,
					sizeDelta,
					...(c.commenthidden ? {} : c.comment ? { comment: c.comment } : {}),
					...(c.minor ? { minor: true } : {}),
					...(c.bot ? { bot: true } : {}),
					...(c.new ? { isNew: true } : {}),
					...(c.redirect ? { redirect: true } : {}),
					...(c.unpatrolled ? { unpatrolled: true } : {}),
